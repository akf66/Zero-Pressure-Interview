...
| 服务 | 职责 | 技术选型 |
|------|------|----------|
| **API Gateway** | HTTP入口、路由转发、JWT鉴权、限流 | Hertz |
| **User 服务** | 用户注册登录、信息管理、简历管理 | Kitex RPC |
| **Agent 服务** | AI对话编排、面试流程控制、评估生成 | Kitex RPC |
...
用户请求 → API Gateway (Hertz HTTP)
              ...
              ├── /api/interview/* → Agent Service (Kitex RPC)├── /api/question/* → Question Service (Kitex RPC)
              └── /api/file/*    → Storage Service (Kitex RPC)
```
